...
public class ClassLoaderWrapper {
	...
	public Iterator<URL> getResourceAsURLs(String resource, ClassLoader classLoader) throws IOException {
	...
	InputStream getResourceAsStream(String resource, ClassLoader[] classLoader) {
		...
		if (null == returnValue) {
			returnValue = ClassLoader.getSystemResourceAsStream(resource);
			if (null == returnValue) {
				returnValue = ClassLoader.getSystemResourceAsStream('/' + resource);
			}
		}
	...
	URL getResourceAsURL(String resource, ClassLoader[] classLoader) {
		...
		// try again if we didn't find the resource
		if (null == returnValue) {
			returnValue = ClassLoader.getSystemResource(resource);
			if (null == returnValue) {
				returnValue = ClassLoader.getSystemResource('/' + resource);
			}
		}
	...
	Iterator<URL> getResourceAsURLs(String resource, ClassLoader[] classLoader) throws IOException {

        EnumerationIterator<URL> iterator = new EnumerationIterator<URL>();
		...
		for (ClassLoader cl : classLoader) {
			...
			if (null != cl) {
				...
				// finding it, so stop looking already.
				if (null != returnValue) {
					iterator.addEnumeration(returnValue);
					return iterator;
				}
		...
		// try again if we didn't find the resource
		if (null == returnValue) {
			returnValue = ClassLoader.getSystemResources(resource);
			if (null == returnValue) {
				returnValue = ClassLoader.getSystemResources('/' + resource);
			}
		}
